...
## PARTE 3 (REESCRITA): DOC-1 Rico e Completo para IA Externa
...
O DOC-1 que sai do sistema precisa ser **tão rico quanto o DOC-2 da metodologia manual**.
A IA que vai receber esse arquivo precisa conseguir implementar a landing page **sem ambiguidade nenhuma**:
sem precisar inventar nada, sem precisar perguntar nada, sem campos em branco.

O que estava errado na versão anterior: o DOC-1 saía com o briefing e a copy, mas sem:
- Regras técnicas da stack
- Regras de design e viewport
- Especificações de componentes
- Rastreamento completo
- Checklist de entrega
- Regras de acessibilidade
- Páginas adicionais (/links, /404, etc)

O que precisa acontecer: o DOC-1 que sai do LandingAI precisa **embutir automaticamente**
todas as Regras Fixas Adsgator (que hoje estão no DOC-2, Parte 3) + os dados do projeto.

---

## 1. ESTRUTURA COMPLETA DO DOC-1

O DOC-1 tem **duas partes distintas** que o sistema monta ao gerar:

```
DOC-1
├── PARTE A — Dados do Projeto (preenchidos pelo usuário no sistema)
│   ├── Instrução Mestre (1 única instrução clara)
│   ├── Resumo do Projeto
│   ├── Análise de Intenção (3 dores)
│   ├── Metadados de SEO
│   ├── Fluxo Selecionado (blocos confirmados)
│   ├── Copy Completa (por bloco, formato padrão)
│   └── Direção de Arte
│
└── PARTE B — Regras Fixas Adsgator (embutidas automaticamente, imutáveis)
    ├── Stack Técnica
    ├── Componentes Globais
    ├── Padrão de Assets
    ├── Design de Viewport
    ...
    ├── Animações
    ├── Acessibilidade
    ├── Responsivo por Seção
    ├── Integrações Técnicas
    ├── Schema.org
    ├── Páginas Adicionais (/links, /404, /politica)
    ├── Proibições Visuais Globais
    └── Checklist de Entrega
```

A **Parte B nunca muda** entre projetos — é gerada igual sempre.
A **Parte A muda** com cada projeto.

---

## 2. IMPLEMENTAÇÃO: NOVO GERADOR DO DOC-1

### 2.1 Criar `assets/js/screens/gerar-doc1.js`

Este arquivo substitui qualquer lógica atual de geração do DOC-1.

```javascript
// assets/js/screens/gerar-doc1.js
// ============================================================
// Gerador do DOC-1 completo para IA externa
// ============================================================
...
// Atualizar aqui quando a metodologia Adsgator mudar
// ============================================================
...
function montarParteA(state) {
  ...
  const nomeCliente = briefing?.nome_cliente || 'Não informado';
  const slug = projetoAtual?.slug || 'projeto';

  // Monta link WA com mensagem pré-formatada
  const whatsapp = briefing?.whatsapp?.replace(/\D/g, '') || '';
  const msgWA = encodeURIComponent(briefing?.mensagem_wa || `Olá! Vi o site e quero saber mais.`);
  const linkWA = whatsapp ? `https://wa.me/${whatsapp}?text=${msgWA}` : '[preencher]';

  // Integrações ativas (baseadas no briefing)
  const integracoes = [];
  if (briefing?.google_business && briefing?.google_business_reviews >= 10) {
    integracoes.push(`[x] Google Reviews widget — ${briefing.google_business_nota}`);
  } else {
    integracoes.push(`[ ] Google Reviews widget — ${briefing?.google_business ? 'perfil existe, mas sem avaliações suficientes' : 'sem perfil'}`);
  }
  if (briefing?.instagram) {
    integracoes.push(`[x] Instagram Feed — ${briefing.instagram_handle}`);
  } else {
    integracoes.push(`[ ] Instagram Feed — sem perfil ativo`);
  }
  if (briefing?.modalidade === 'presencial' && briefing?.endereco_autorizado) {
    integracoes.push(`[x] Google Maps embed — ${briefing.endereco_completo}`);
  } else {
    integracoes.push(`[ ] Google Maps embed — atendimento não presencial ou endereço não autorizado`);
  }
  if (briefing?.formulario_contato) {
...
| **Objetivo da Página** | ${briefing?.objetivo_pagina || 'Conversão para WhatsApp'} |
| **Público-Alvo Principal** | ${briefing?.avatar_principal || '—'} |
| **Tom de Voz** | ${briefing?.tom_voz || '—'} |
| **Objetivo de Conversão** | ${briefing?.objetivo_conversao || 'Mensagem no WhatsApp'} |
| **Cidade / Região** | ${briefing?.cidade || '—'} |
| **Modalidade** | ${briefing?.modalidade || '—'} |
| **Número WhatsApp** | ${briefing?.whatsapp || '—'} |
| **Mensagem Pré-Preenchida** | ${briefing?.mensagem_wa || 'Olá! Vi o site e quero saber mais.'} |
| **Link CTA Principal** | ${linkWA} |
| **GTM ID** | ${briefing?.gtm_id || 'GTM-XXXXXXX (inserir antes do deploy)'} |
| **Domínio Previsto** | ${briefing?.dominio || '[dominio-do-cliente].com.br'} |
| **CNPJ** | ${briefing?.cnpj || 'Não informado'} |
...
**Assets Disponíveis:**
\`\`\`
Foto do profissional:   ${briefing?.foto_profissional ? 'Sim — ' + briefing.foto_qualidade : 'Não'}
Logo da marca:          ${briefing?.logo ? 'Sim — ' + briefing.logo_formato : 'Não'}
Depoimentos:            ${briefing?.depoimentos ? 'Sim — ' + briefing.depoimentos_qtd + ' depoimentos' : 'Não'}
Google Business:        ${briefing?.google_business ? 'Sim — ' + briefing.google_business_nota : 'Não'}
Instagram ativo:        ${briefing?.instagram ? 'Sim — ' + briefing.instagram_handle : 'Não'}
Endereço físico:        ${briefing?.endereco_autorizado ? 'Sim — ' + briefing.endereco_completo : 'Não / Não autorizado'}
\`\`\`
...
**Cores da Marca:**
\`\`\`
Cor principal:    ${artDirection?.corPrincipal || 'Sem identidade definida'}
Cor secundária:   ${artDirection?.corSecundaria || 'Não existe'}
Logo disponível:  ${briefing?.logo ? (briefing.logo_formato || 'Sim') : 'Não'}
Observações:      ${artDirection?.corObservacoes || '—'}
\`\`\`

**Direção Geral:**
\`\`\`
Estilo desejado:    ${artDirection?.estilo || '—'}
O que não quero:    ${artDirection?.naoQuero || '—'}
Tema:               ${artDirection?.tema || 'IA decide com base nas referências'}
Intensidade visual: ${artDirection?.intensidade || '—'}
\`\`\`

**Referências Visuais:**
${(artDirection?.referenciasVisuais || []).map((r, i) => `
Referência ${i + 1}: ${r.link || r.nome || '—'}
O que me atraiu: ${r.descricao || '—'}
`).join('') || 'Nenhuma referência adicionada'}

**Referências do Nicho:**
${artDirection?.referenciasNicho || '—'}

**Direção do Footer:**
\`\`\`
Tom visual:        ${artDirection?.footer_tom || '—'}
Elemento âncora:   ${artDirection?.footer_ancora || '—'}
O que deve conter: logo, redes sociais, política de privacidade, CNPJ${briefing?.cnpj ? ' (' + briefing.cnpj + ')' : ''}, logo Adsgator
 ...
 */
function gerarDoc1Completo() {
  const parteA = montarParteA(appState);
  return parteA + REGRAS_FIXAS_ADSGATOR;
}
 ...
 */
function baixarDoc1() {
  const { projetoAtual, briefing } = appState;
  const nomeCliente = briefing?.nome_cliente || 'projeto';
  const slug = projetoAtual?.slug || nomeCliente.toLowerCase().replace(/\s+/g, '-');

  const conteudo = gerarDoc1Completo();
 ...
 */
async function copiarDoc1() {
  try {
    const conteudo = gerarDoc1Completo();
    await navigator.clipboard.writeText(conteudo);
    showToast('✅ DOC-1 copiado para o clipboard!', 'success');
  } catch (err) {
    showToast('❌ Erro ao copiar. Use o botão de download.', 'error');
  }
}
...
## 3. CAMPOS NECESSÁRIOS NO STATE

Para o DOC-1 sair completo, o `appState` precisa ter estes campos mapeados do briefing e dos steps:

```javascript
// assets/js/01-state.js — campos necessários

appState = {
  projetoAtual: {
    id, nome, slug, dataCriacao, dataAtualizacao, status
  },
  ...
  briefing: {
    ...
    // Contato e Conversão
    whatsapp,           // só dígitos, com DDI
    mensagem_wa,        // mensagem pré-preenchida
    email,
    objetivo_conversao,
    gtm_id,
    dominio,
    ...
    cidade,
    endereco_completo,
    endereco_autorizado, // boolean

    // Presença digital
    google_business,    // boolean
    google_business_nota, // "4.8★ com 127 avaliações"
    google_business_reviews, // number (para validar >= 10)
    instagram,          // boolean
    instagram_handle,   // @handle
    foto_profissional,  // boolean
    foto_qualidade,     // "boa qualidade" | "qualidade média"
    logo,               // boolean
    logo_formato,       // "SVG" | "PNG"
    depoimentos,        // boolean
    depoimentos_qtd,    // number
    formulario_contato, // boolean
  ...
  intencao: {
    dor1_real, dor1_busca, dor1_resultado,
    dor2_real, dor2_busca, dor2_resultado,
    dor3_real, dor3_busca, dor3_resultado,
  },
  ...
  estruturaCopyBlocos: [
    {
      id, nome, tipo,
      objetivo, copy, integracao, visual
    }
  ],
  ...
  artDirection: {
    ...
    menu_estilo,
    menu_especial,
    referenciasVisuais: [{ id, tipo, link, nome, descricao }],
    referenciasNicho,
  }
};
```
...
## 4. GARANTIR QUE OS CAMPOS EXISTEM NO INTAKE

Verificar se o formulário de INTAKE coleta todos esses campos. Campos que podem estar faltando:
...
✅ modalidade
✅ cidade
⚠️ proposta_valor       → verificar se tem campo ou é gerado pela IA
⚠️ mensagem_wa          → verificar se tem campo
⚠️ gtm_id              → verificar se tem campo (pode ser step separado)
⚠️ dominio             → verificar se tem campo
⚠️ cnpj               → verificar se tem campo
⚠️ foto_qualidade      → verificar se é apenas boolean ou tem detalhes
⚠️ google_business_reviews → verificar se coleta número
⚠️ dor_principal       → verificar se é coletada no intake ou apenas na análise
```

Campos marcados com ⚠️ precisam ser verificados no código de INTAKE e adicionados se não existirem.

---

## 5. BOTÕES NA TELA DE REVISÃO

Adicionar ou atualizar botões na tela de Revisão Final em `index.html`:

```html
<!-- Seção: Exportar DOC-1 -->
<div class="revisao-exportar">
  <h3>Exportar DOC-1</h3>
  <p>
    Pronto para enviar para uma IA externa (Claude, Gemini, etc) gerar o Documento de Implementação.
  </p>

  <div class="exportar-buttons">
    <button
      class="btn btn-primary btn-lg"
      onclick="window.LandingAI.doc1.baixarDoc1()">
      📥 Baixar DOC-1 (.md)
    </button>

    <button
      class="btn btn-secondary"
      onclick="window.LandingAI.doc1.copiarDoc1()">
...
```javascript
import * as doc1Module from './screens/gerar-doc1.js';
...
- [ ] Criar `assets/js/screens/gerar-doc1.js` com todo o código acima
- [ ] Verificar campos do INTAKE e adicionar os que estão faltando
- [ ] Mapear todos os campos no `appState` (01-state.js)
- [ ] Registrar módulo no objeto global (app.js)
- [ ] Atualizar botões na tela de Revisão
- [ ] Testar geração de DOC-1 com projeto preenchido
- [ ] Verificar que Parte A contém dados reais (não placeholders)
- [ ] Verificar que Parte B (Regras Fixas) está completa
- [ ] Testar download do arquivo
- [ ] Testar cópia para clipboard
- [ ] Abrir arquivo gerado e validar que está pronto para enviar para IA

---

## 8. TESTE DE VALIDAÇÃO DO DOC-1

Após gerar o DOC-1, validar manualmente:

| Item | Esperado |
|------|---------|
| Nome do profissional | Real, não placeholder |
| WhatsApp | Só dígitos com DDI |
| Análise de Intenção | 3 dores com dor real, busca e resultado |
| Metadados SEO | Title ≤ 60 chars, description ≤ 160 chars |
| Copy blocos | Mínimo 5 blocos com copy real completa |
| Direção de Arte | Cor, estilo, referências preenchidas |
| Integrações | Marcadas corretamente com base no briefing |
| Regras Fixas | Presente e completa (não encurtada) |
| Tamanho total | ≥ 10.000 palavras |
| Placeholders | Zero ocorrências de "[definir depois]" |
